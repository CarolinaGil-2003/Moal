    ...
    <div class="row">
        <div class="col-md-10 offset-md-1">
            <div class="mt-1">
                ...
                if (isset($_GET['q'])) {
                    $q = $_GET['q'];
                    $query = "SELECT id, name, photo_url, size, createdAt FROM `Products` WHERE MATCH(name, description, category_name) AGAINST('$q' IN BOOLEAN MODE);";
                } else {
                    $query = "SELECT id, name, photo_url, size, createdAt FROM `Products`";
                }
                $result = mysqli_query($conn, $query);
                if (mysqli_num_rows($result) == 0) { ?>
                    <h1 class=text-center><?= (isset($_GET['q'])) ? 'No hay productos que coincidan con la busqueda' : 'Todavía no hay productos para mostrar' ?></h1>
                <?php
                } else { ?>
